...
import 'package:faceunity_ui/Tools/FaceUSharedPreference.dart';
...
class FUBeautyFilterViewModel extends BaseViewModel {
  FUBeautyFilterViewModel(FaceUnityModel dataModel) : super(dataModel) {
    Future.delayed(Duration(milliseconds: 0),_upParams);
  }

  Future<void> _upParams() async {
    ...
    String commonPre = FUImageTool.getImagePathWithRelativePathPre("Asserts/beauty/filter/");
    ...
    for (var i = 0; i < titlesKey.length; i++) {
      ...
      double _value = await FaceUSharedPreference.getValue(titleKey,0.4);
      BaseModel model = BaseModel(imagePaths[i], titleAndImagePath[titleKey], _value);
      model.ratio = 1.0;
  ...
  @override
  //具体选中哪一个由子类决定
  Future<void> sliderValueChange(double value) async {
    await super.sliderValueChange(value);
